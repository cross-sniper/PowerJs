...
it is built ontop of duktape, and has a module api, that allows for extensability

what this module does, is to provide a way to access the raylib api from the js engine
allowing you to make games using raylib + javascript(ecmascript, since duktape is made for that)
*/
...
static duk_ret_t initwin(duk_context* ctx){
	...
	if(w <= 0 || h <= 0){
		duk_error(ctx, DUK_ERR_TYPE_ERROR, "invalid size");
		return 0;
	}
